    ...
    <div className="min-h-screen flex items-center justify-center px-4 py-8 relative overflow-hidden"
      ...
      <div className="flex flex-col lg:flex-row items-center gap-12 w-full max-w-4xl relative z-10">
        ...
        <div className="flex flex-col items-center flex-1">
          <div className="relative w-44 h-44 flex items-center justify-center mb-6">
            {[0, 12, 24].map((inset, i) => (
              <div key={i} className="absolute rounded-full" style={{
                inset,
                border: `${1.5 - i * 0.5}px dashed rgba(${i === 0 ? '99,102,241' : i === 1 ? '6,182,212' : '168,85,247'},${0.4 - i * 0.1})`,
                animation: `${i % 2 === 0 ? 'spin' : 'spinRev'} ${12 - i * 3}s linear infinite`
              }} />
            ))}
          ...
          <h1 className="text-2xl font-semibold mb-2" style={{ color: "#0f0f13", letterSpacing: "-0.02em" }}>
            AI{" "}
            <span style={{
          ...
          <p className="text-sm text-center max-w-48 mb-6" style={{ color: "#6b7280", lineHeight: 1.6 }}>
          ...
          <div className="flex flex-col gap-2.5 w-full max-w-52">
            {[
              ...
              <div key={i} className="flex items-center gap-2.5 px-3.5 py-2.5 rounded-xl"
                ...
                <span className="text-xs" style={{ color: "#374151" }}>{f.label}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="w-full max-w-sm flex-shrink-0">
          <div className="rounded-2xl p-8" style={{
            ...
            <div className="text-center mb-7">
              ...
              <h2 className="text-xl font-semibold mb-1.5" style={{ color: "#0f0f13" }}>Welcome back</h2>
              <p className="text-sm" style={{ color: "#6b7280" }}>Sign in to your account</p>
            </div>

            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-xs mb-1.5" style={{ color: "#374151" }}>Email address</label>
                <input
                  type="email" name="email" placeholder="you@example.com"
                  value={formData.email} onChange={handleChange} required
                  className="w-full rounded-xl px-3.5 py-3 text-sm outline-none transition-all"
                  style={{ background: "#f8f8ff", border: "1px solid rgba(99,102,241,0.2)", color: "#0f0f13", fontFamily: "inherit" }}
        ...
        <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
          ...
          <circle cx="12" cy="12" r="3"/>
              ...
              <button type="submit" disabled={loading}
                ...
                style={{ background: "linear-gradient(135deg,#6366f1,#06b6d4)", opacity: loading ? 0.7 : 1, fontFamily: "inherit" }}>
            ...
            <p className="text-center text-sm" style={{ color: "#6b7280" }}>
              ...
              <Link href="/register" style={{ color: "#6366f1", textDecoration: "none" }}>Register</Link>
            </p>
          </div>
        </div>
      </div>
